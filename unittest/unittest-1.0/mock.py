...
class _Call(tuple):
    ...
    # 该方法用于返回一个新的_Call对象.
    # 补充:
    # 1. 首先call的比较方式是根据两者的参数进行比较, 而不是以对象的id来进行比较.
    # 2. 其次call的主要目的是保留调用时签名(参数).
    # 3. 最后call的内部逻辑主要是保障, 不同的参数形式, 最终都能同质化, 使其能够在最大程度上进行一致性比较.
    #
    # 所以这里 __call__ 主要的作用是, 通过调用 call(参数) 生成一个新的对象, 用于与目标mock对象的参数的一致性进行比较.
    # 例如:
    # mock(1)
    # mock.assert_has_calls([call(1)])
    # 这里call(1)的作用是, 用于比较 mock(1) 调用时提供的参数是否与call(1)的参数一致.
    ###################################################################################################################
    def __call__(self, /, *args, **kwargs):
        ...
        # 注意: 由于 __eq__ 要求 两个对象的比较 parent 要一致,
